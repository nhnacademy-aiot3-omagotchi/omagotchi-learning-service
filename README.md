...
교육 기수, 학습, 출결, 공간·팀과 게이미피케이션 데이터를 소유하는 서비스입니다. 현재 기반에는 PostgreSQL 18.1, Flyway와 첫 `cohorts` Migration이 포함되어 있습니다.
...
테스트는 Flyway V1 적용, `learning_service.cohorts` 생성과 PostgreSQL 버전을 확인합니다.
...
다른 서비스가 소유한 데이터에는 Foreign Key와 JPA 연관관계를 만들지 않습니다. Identity의 사용자는 `userId` 값으로만 논리 참조합니다.
