...
public class OccupancyParticipantService {
     ...
     *                           공간 없음(404), 종료된 점유·정원 초과·이미 참여 중(409)
     */
    @Transactional
    public void add(Long spaceId, UUID targetUserId, UUID requesterUserId) {
        ...
        // 락 안에서 센다. 락 밖에서 세면 잔여 1석에 둘이 동시에 들어와 정원을 넘는다.
        // 정원은 "최대 N행"이라 유니크 인덱스로 표현할 수 없어 이 카운트가 유일한 방어선이다.
        ...
        participantRepository.find(locked.getId(), targetUserId)
                .ifPresentOrElse(
    ...
    public void remove(Long spaceId, UUID targetUserId, UUID requesterUserId) {
        ...
        OccupancyParticipant participant = participantRepository
                .find(locked.getId(), targetUserId)
     ...
     * ACTIVE 멤버십이 아니면 기수를 특정할 수 없어 정합 검증이 성립하지 않는다.</p>
     */
    private Long cohortIdOf(Long membershipId) {
        return cohortMembershipQueryService.findActiveMembership(membershipId)
                .map(CohortMembershipView::cohortId)
                .orElseThrow(() -> new BusinessException(OccupancyErrorCode.DIFFERENT_COHORT));
    }
}
