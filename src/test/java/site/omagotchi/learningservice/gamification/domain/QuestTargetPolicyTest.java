...
class QuestTargetPolicyTest {
    ...
    void multipliesPredictionByChallengeCoefficient() {
        // 4h * 1.1 = 4.4h = 15840s
        int target = QuestTargetPolicy.targetSecondsOf(4.0, COEFFICIENT, MIN_SECONDS, MAX_SECONDS);

        assertEquals(15_840, target);
    }
    ...
    void clampsAfterMultiplyingNotBefore() {
        ...
        int target = QuestTargetPolicy.targetSecondsOf(11.5, COEFFICIENT, MIN_SECONDS, MAX_SECONDS);

        assertEquals(MAX_SECONDS, target);
    }
    ...
    void neverFallsBelowMinimum() {
        ...
        int target = QuestTargetPolicy.targetSecondsOf(0.0, COEFFICIENT, MIN_SECONDS, MAX_SECONDS);

        assertEquals(MIN_SECONDS, target);
    }
    ...
    void raisesSmallPredictionToMinimum() {
        // 1h * 1.1 = 3960s < 12600s
        int target = QuestTargetPolicy.targetSecondsOf(1.0, COEFFICIENT, MIN_SECONDS, MAX_SECONDS);
