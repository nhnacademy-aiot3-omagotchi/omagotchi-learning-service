...
class EndedMembershipPresenceCleanupTest {
    ...
    @Test
    @DisplayName("점유·회의를 먼저 정리하고 같은 종료 시각으로 출결을 마감한다")
    void cleansOccupancyBeforeAttendance() {
        ...
        order.verify(attendanceCleanup).cleanUp(MEMBERSHIP_ID, ENDED_AT);
    }
    ...
    void occupancyFailureDoesNotBlockAttendance() {
        ...
        verify(attendanceCleanup).cleanUp(MEMBERSHIP_ID, ENDED_AT);
    }
    ...
    void attendanceFailureIsIsolated() {
        willThrow(new IllegalStateException("출결 정리 실패"))
                .given(attendanceCleanup).cleanUp(MEMBERSHIP_ID, ENDED_AT);
