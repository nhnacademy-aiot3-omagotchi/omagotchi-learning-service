...
public class StudyPatternTools implements AiToolProvider {
    ...
    @Tool(description = """
            ...
            "내 공부 습관 어때?", "요즘 나 공부 잘하고 있어?", "언제 공부하는 게 좋아?" 처럼 학습 패턴의 진단이나 조언이 필요할 때 사용하세요.
            총 학습 시간이나 학습일 수만 묻는 질문에는 이 도구가 아니라 학습 시간 요약 도구를 사용하세요.
            ...
            ### 응답 해석 규칙
            - status가 "NO_DATA"면: 그 기간에 학습 기록이 없는 것입니다. 오류가 아니니 첫 학습 시작을 권유하는 코칭으로 답하세요.
            - typicalStartTime: 이 사용자가 보통 하루의 공부를 시작하는 시각입니다. 날마다 들쭉날쭉한지 일정한지는 알 수 없으니 "보통 이 무렵 시작"으로만 말하세요.
            - bestStartHour: 이 시각에 시작한 세션들의 누적 공부량이 가장 많았다는 뜻입니다. "이 시간대 내내 공부했다"가 아니라 "이 시각에 시작하면 잘 굴러간다"로 해석해서 시작 시각을 추천할 때 사용하세요.
            - averageSessionMinutes가 짧고 sessionCount가 많으면 공부가 잘게 끊기고 있다는 신호입니다. 몰입 블록을 만들라고 조언할 근거로 쓰세요.
            - focusDensityPercent: 앉아 있던 시간 중 실제 공부한 비율입니다. status가 "OK"일 때만 의미가 있습니다(NO_DATA면 0이 내려오니 언급하지 마세요). 총 시간보다 이 값을 먼저 보세요. 낮으면(70% 미만) "더 오래"가 아니라 "끊김을 줄이라"고 조언하세요.
